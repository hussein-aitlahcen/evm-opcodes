...
data OpcodeSpec = OpcodeSpec
  { -- | Numeric encoding of opcode
    opcodeEncoding :: !Word8,
    -- | Number of items opcode places on stack (α)
    opcodeAlpha :: !Word8,
    -- | Number of items opcode removes from stack (δ)
    opcodeDelta :: !Word8,
    -- | Printable name for opcode, e.g. @"add"@
    opcodeName :: !Text
  }
  deriving (Eq, Show)
